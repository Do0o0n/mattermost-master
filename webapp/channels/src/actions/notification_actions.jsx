    ...
    return async (dispatch, getState) => {
        ...
        if (notify) {
            ...
            if (sound && !isDesktopApp() && !isMobileApp()) {
                if (isBuzz) {
                    NotificationSounds.ding('Buzz'); // تشغيل "Buzz" في حالة معينة
                } else {
                    NotificationSounds.ding(soundName); // تشغيل الصوت المخصص للمستخدم في الحالات الأخرى
                }
            }
            return result;
        }
...
export const notifyMe = (title, body, channel, teamId, silent, soundName, url) => async (dispatch) => {
    ...
    try {
        ...
        const notificationResult = await dispatch(showNotification({
            title,
            body,
            ...
            onClick: () => {
                window.focus();
                getHistory().push(url);
                if (body.includes('BUZZMESSAGE')) {
                    window.focus(); // التركيز على نافذة المتصفح
                    getHistory().push(url); // فتح الرابط المحدد
        ...
        if (isBuzz) {
            ...
            NotificationSounds('Buzz');
        }

        return notificationResult; // Return the result of showing the notification
    } catch (error) {
