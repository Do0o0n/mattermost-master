...
export default class FilePreviewModal extends React.PureComponent<Props, State> {
    ...
    static getDerivedStateFromProps(props: Props, state: State) {
        ...
        // قائمة بامتدادات الصور الشائعة
        const imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp', 'tiff'];

        // التحقق مما إذا كان الملف صورة أو PDF لعرض شريط التكبير
        if (props.fileInfos[state.imageIndex] &&
            (imageExtensions.includes(props.fileInfos[state.imageIndex].extension.toLowerCase()) ||
    ...
    showImage = (id: number) => {
        const fileInfo = this.props.fileInfos[id];
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const PDF_DEFAULT_SCALE = 2;
        const isPDF = fileInfo.extension === FileTypes.PDF;

        this.setState({
            imageIndex: id,
            scale: {
                ...this.state.scale,
                [id]: isPDF ? PDF_DEFAULT_SCALE : this.state.scale[id],
            },
    ...
    loadImage = (index: number) => {
        ...
        if (fileType === FileTypes.IMAGE && isFileInfo(fileInfo)) {
            ...
            if (fileInfo.has_preview_image) {
                ...
                // بعض الصور (مثل GIF المتحرك) تعرض الملف نفسه وليس معاينة
            ...
            // لا حاجة لتحميل الملفات غير الصور
            this.handleImageLoaded(index);
        }
    };
    ...
    render() {
        ...
        // التحقق من تحميل الصورة لعرض المحتوى الصحيح
        if (this.state.loaded[this.state.imageIndex]) {
            const imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp', 'tiff'];

            if (imageExtensions.includes(fileInfo.extension.toLowerCase())) {
                content = (
                    <ImagePreview
                        fileInfo={fileInfo as FileInfo}
                        canDownloadFiles={this.props.canDownloadFiles}
                        scale={this.state.scale[this.state.imageIndex]} // تمرير حالة التكبير
                    />
                );
                zoomBar = (
                    <PopoverBar
            ...
            // عرض مؤشر التحميل إذا كانت الصورة قيد التحميل
        ...
        return (
            <Modal
                ...
                className='modal-image file-preview-modal modal-fils'
