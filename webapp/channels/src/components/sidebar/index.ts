...
function mapStateToProps1(state: GlobalState) {
    const currentTeam = getCurrentTeam(state);
    const unreadFilterEnabled = isUnreadFilterEnabled(state);
    const userGroupsEnabled = isCustomGroupsEnabled(state);
...
function mapDispatchToProps1(dispatch: Dispatch) {
    return {
        actions: bindActionCreators({
            clearChannelSelection,
            fetchMyCategories,
...
// Export Sidebar2 component
export const ConnectedSidebar2 = connect(mapStateToProps1, mapDispatchToProps1)(Sidebar2);
