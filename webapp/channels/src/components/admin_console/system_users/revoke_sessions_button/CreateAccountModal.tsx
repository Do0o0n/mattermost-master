...
const CreateAccountModal = ({onCustomizeHeader}: SignupProps) => {
    const intl = useIntl();
    ...
    const usedBefore = useSelector((state: GlobalState) =>
        (!inviteId && !loggedIn && token ? getGlobalItem(state, token, null) : undefined),
    );
    ...
    const noOpenServer = false; // جعل هذا القيمة ثابتة
    ...
    const {error: passwordInfo} = isValidPassword(
        '',
        getPasswordConfig(config),
        intl,
    );
    ...
    const handleSignupSuccess = async (
        ...
        // إخفاء الكمبوننت بعد نجاح التسجيل
        setIsVisible(false);
    };
    ...
    if (!isVisible) {
        return null; // إخفاء الكمبوننت عندما تكون الحالة `isVisible` هي false
    }
    ...
    const getContent = () => {
        ...
        return (
            <>
                ...
                />
                <div className='signup-body-actions'>
                    <div
                                    ...
                                    {getExternalSignupOptions().map(
                                        (option) => (
                                            <ExternalLoginButton
                                                key={option.id}
                                                direction={
                                                    enableSignUpWithEmail ? undefined : 'column'
                                                }
                                                {...option}
                                            />
                                        ),
