...
const ForwardPostModal = ({onExited, post}: Props) => {
    ...
    const isPrivateConversation = channel?.type === General.PRIVATE_CHANNEL;
    ...
    const handleSubmit = () => {
        ...
        // التحقق من نوع القناة: محادثة مباشرة أو قناة عامة
        ...
        // فتح محادثة مباشرة إذا كان نوع القناة DM_CHANNEL
        return Promise.resolve().then(() => {
            if (type === Constants.DM_CHANNEL && userId) {
                // فتح المحادثة مع المستخدم باستخدام openDirectChannelToUserId
                return dispatch(openDirectChannelToUserId(userId));
            }
            return {data: false} as ActionResult;
        }).then(({data}) => {
            if (data) {
                // تحديث معرف القناة إذا تم فتح المحادثة بنجاح
                channelToForward.details.id = data.id;
            }

            // إرسال الرسالة بعد فتح المحادثة أو تحديد القناة
            ...
            // إذا كانت القناة من النوع OPEN_CHANNEL، يتم الانضمام للقناة
            ...
            // فقط التحويل إذا لم تكن القناة محادثة خاصة
            if (!isPrivateConversation) {
                return dispatch(switchToChannel(channelToForward.details));
            }
            return {data: false};
        }).then(() => {
            // إخفاء النافذة بعد التحويل
            onHide();
        }).catch((result) => {
            if (result?.error) {
    ...
    return (
        <GenericModal
            className='a11y__modal forward-post texetheader'
