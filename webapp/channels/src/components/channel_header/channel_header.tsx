...
export type Props = {
    ...
    actions: {
        performSearch (channelId: string): void;
...
class ChannelHeader extends React.PureComponent<Props, State> {
    ...
    render() {
            ...
            if (this.props.isLastActiveEnabled && this.props.lastActivityTimestamp && this.props.timestampUnits) {
                    ...
                    <span className='header-status__text'>
                        <span
                            ...
                            style={{
                                ...
                                color: channel.status === 'offline' ? '#d24b4e' : (channel.status === 'away' ? '#f5ab00' : '#00987e'),
                            }}
                        >
                            {channel.status === 'offline' && 'غير متصل'}
                            {channel.status === 'away' && 'خامل'}
                            {channel.status === 'online' && 'متصل'}
                        </span>
                ...
                dmHeaderTextStatus = (
                    <span className='header-status__text'>
                        <span
                            className='last-active-text'
                            style={{color: channel.status === 'online' ? '#00987e' : '#d24b4e'}}
                        >
                            {channel.status === 'offline' && 'غير متصل'}
                            {channel.status === 'away' && ' خامل'}
                            {channel.status === 'online' && 'متصل'}
        ...
        const channelFilesIcon2 = <i className='fas fa-link'/>;
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    <HeaderIconWrapper
                                        iconComponent={channelFilesIcon2}
                                        ariaLabel={true}
                                        buttonClass={channelFilesIconClass}
                                        buttonId={'channelHeaderMessagesButton'}
                                        onClick={async () => {
                                            if (channel && this.props.actions.showPinnedPosts) {
                                                await this.props.actions.showPinnedPosts(channel.id);

                                                // const prependToggle = (/^\s*https?:\/\/.*$/).lastIndex;
                                                this.props.actions.performSearch(`channel:${channel.id}`);
                                            }
                                        }}
                                        tooltipKey={'channelMessages'}
                                    />
