...
export class UserSettingsGeneralTab extends PureComponent<Props, State> {
    ...
    createEmailSection = () => {
        const {formatMessage} = this.props.intl;
        const lockTeammateNameDisplay1 = this.props.lockTeammateNameDisplay; // افترض أن lockTeammateNameDisplay1 تم تمريره كـ prop
        ...
        if (active) {
            ...
            const helpText = !lockTeammateNameDisplay1 ? (
            ...
            ) : null; // لا تعرض أي نصوص إذا كانت lockTeammateNameDisplay1 صحيحة

            let submit = null;

            if (this.props.user.auth_service === '') {
                // eslint-disable-next-line no-negated-condition
                if (!lockTeammateNameDisplay1) {
                    inputs.push(
                            ...
                            <div className='form-group'>
                                ...
                                <div className='col-sm-7'>
                                    <input
                                        ...
                                        disabled={lockTeammateNameDisplay1} // تعطيل حقل الإدخال بناءً على lockTeammateNameDisplay1
                                    />
                                </div>
                            </div>
                        </div>,
                    ...
                    // عند تفعيل lockTeammateNameDisplay1، لا تعرض أي حقول نصية
                    inputs.push(
                        <div
                            key='emailLockedInfo'
                ...
                const oauthMessages = {
                    [Constants.GITLAB_SERVICE]: 'emailGitlabCantUpdate',
                    [Constants.GOOGLE_SERVICE]: 'emailGoogleCantUpdate',
                    [Constants.OFFICE365_SERVICE]: 'emailOffice365CantUpdate',
                    [Constants.OPENID_SERVICE]: 'emailOpenIdCantUpdate',
                    [Constants.LDAP_SERVICE]: 'emailLdapCantUpdate',
                    [Constants.SAML_SERVICE]: 'emailSamlCantUpdate',
                };

                const messageKey = oauthMessages[this.props.user.auth_service];
                ...
                inputs.push(
                    ...
                    >
                        <div className='setting-list__hint pb-3'>
                            <FormattedMessage
                                id={`user.settings.general.${messageKey}`}
                                defaultMessage='Login occurs through {provider}. Email cannot be updated. Email address used for notifications is {email}.'
                                values={{
                                    provider: this.props.user.auth_service,
        ...
        if (this.props.user.auth_service === '') {
            ...
            describe = (
                <FormattedMessage
                    id={`user.settings.general.login${this.props.user.auth_service}`}
                    defaultMessage='Login done through {provider} ({email})'
                    values={{
                        provider: this.props.user.auth_service,
                        email: this.state.originalEmail,
                    }}
                    ...
                    >
                        ...
                        <div className='col-sm-7'>
                            <input
                                ...
                                disabled={lockTeammateNameDisplay1} // تعطيل حقل الإدخال
                            />
                        </div>
                    </div>,
                );
    ...
    createNicknameSection = () => {
        ...
        if (active) {
            ...
            // تعديل النص بناءً على قيمة lockTeammateNameDisplay1
            if (lockTeammateNameDisplay1) {
                extraInfo = (
                    <span>
                        <FormattedMessage
            ...
            } else if ((this.props.user.auth_service === 'ldap' && this.props.ldapNicknameAttributeSet) ||
                (this.props.user.auth_service === Constants.SAML_SERVICE && this.props.samlNicknameAttributeSet)) {
                extraInfo = (
                    <span>
                        <FormattedMessage
                            id='user.settings.general.field_handled_externally'
    ...
    createPositionSection = () => {
        ...
        if (active) {
            ...
            // التحقق مما إذا كان الحقل محجوزًا للتعديل من خلال موفر تسجيل الدخول
            if ((user.auth_service === Constants.LDAP_SERVICE && this.props.ldapPositionAttributeSet) ||
                (user.auth_service === Constants.SAML_SERVICE && this.props.samlPositionAttributeSet)) {
                extraInfo = (
                    <span>
                        <FormattedMessage
                            id='user.settings.general.field_handled_externally'
                ...
                if (lockTeammateNameDisplay1) {
                    positionLabel = ''; // إخفاء التسمية إذا كانت lockTeammateNameDisplay1 صحيحة
                }

                // إضافة الحقول فقط إذا لم يكن lockTeammateNameDisplay1 صحيحًا
                // eslint-disable-next-line no-negated-condition
                if (!lockTeammateNameDisplay1) {
                    inputs.push(
                        <div
