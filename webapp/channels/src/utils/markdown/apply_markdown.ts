...
function applyCodeMarkdown({selectionEnd, selectionStart, message}: ApplySpecificMarkdownOptions) {
    // استخراج النص المحدد
    const codeSnippet = message.slice(selectionStart, selectionEnd);

    // التعرف على نوع اللغة
    const language = detectLanguage(codeSnippet);

    // إذا كان النص متعدد السطور، يضيف ` ``` ` مع نوع اللغة
    ...
    // في حال كان سطر واحد، استخدم شيفرة واحدة فقط
    return applyMarkdownToSelection({selectionEnd, selectionStart, message, delimiter: '`'});
}

function detectLanguage(code: string): string {
    // 1. Java: التحقق من بناء الفئات والدوال العامة في Java
    if ((/\bpublic\s+class\b|\bpublic\s+static\s+void\s+main\(\)|\bSystem\.out\.println\b/).test(code)) {
        return 'java';
    // eslint-disable-next-line brace-style
    }

    // 2. SCSS: التحقق من المتغيرات والأنماط الخاصة بـ SCSS والبنية المتداخلة
    else if ((/\$[a-zA-Z0-9_-]+:|@mixin\b|@include\b|@extend\b|\b&\b|\blighten\b|\bdarken\b|[a-zA-Z0-9_-]+\s*:\s*[^;]+;/).test(code) &&
             !(/\bfunction\b|\bconsole\.log\b|\blet\b|\bconst\b|\bvar\b|=>|\breturn\b/).test(code)) {
        return 'scss';
    // eslint-disable-next-line brace-style
    }

    // 3. JavaScript: شروط خاصة بـ JavaScript، مع استثناءات Go وKotlin
    else if ((/[=]>/).test(code) &&
             !(/\bpackage\s+main\b|\bimport\s+"fmt"|\bfunc\s+main\(\)|\bimport\s+android\.\b|\bfun\b|\bAppCompatActivity\b/).test(code)) {
        return 'javascript';
    // eslint-disable-next-line brace-style
    }

    // 4. Go: التحقق من عناصر Go الأساسية مثل الحزم والدوال
    else if ((/\bpackage\s+main\b|\bimport\s+"fmt"|\bfunc\s+main\(\)/).test(code)) {
        return 'go';
    // eslint-disable-next-line brace-style
    }

    // 5. Swift: التحقق من الدوال والكائنات الخاصة بـ Swift
    else if ((/import\s+UIKit\b/).test(code)) {
        return 'swift';
    // eslint-disable-next-line brace-style
    }

    // 6. Kotlin: التحقق من دوال Kotlin وعناصر واجهة المستخدم في Android
    else if ((/\bfun\b/).test(code)) {
        return 'kotlin';
    // eslint-disable-next-line brace-style
    }

    // 7. Python: التركيز على تعريفات الدوال، الطباعة، والكائنات الذاتية
    else if ((/\bdef\b|\bprint\(|\bclass\b\s+[A-Za-z_][a-zA-Z0-9_]*\b|\bself\b/).test(code)) {
        return 'python';
    // eslint-disable-next-line brace-style
    }

    // 8. C++: التحقق من الدوال الرئيسية والعبارات المؤهلة
    else if ((/#include\s*<|\bstd::\b|\bint\s+main\s*\(\)|\bcout\b/).test(code)) {
        return 'cpp';
    // eslint-disable-next-line brace-style
    }

    // 9. HTML: التحقق من الوسوم والنصوص المشفرة
    else if ((/<[a-zA-Z][\s\S]*>/).test(code)) {
        return 'html';
    // eslint-disable-next-line brace-style
    }
    // eslint-disable-next-line no-trailing-spaces
    
    else if ((/^<\?php/).test(code)) {
        return 'php';
    }
